...
typedef NS_OPTIONS(NSUInteger, SDWebImageOptions) {
     ...
     */
    SDWebImageRetryFailed = 1 << 0,             // 下载失败重试
    
    /**
     * By default, image downloads are started during UI interactions, this flags disable this feature,
     * leading to delayed download on UIScrollView deceleration for instance.
     */
    SDWebImageLowPriority = 1 << 1,             // 滑动过程不下载
    
    /**
     * This flag enables progressive download, the image is displayed progressively during download as a browser would do.
     * By default, the image is only displayed once completely downloaded.
     */
    SDWebImageProgressiveLoad = 1 << 2,         // 渐进下载并展示
     ...
     */
    SDWebImageRefreshCached = 1 << 3,           // 刷新本地缓存
    
    /**
     * In iOS 4+, continue the download of the image if the app goes to background. This is achieved by asking the system for
     * extra time in background to let the request finish. If the background task expires the operation will be cancelled.
     */
    SDWebImageContinueInBackground = 1 << 4,    // 进入后台，开启后台任务继续下载，后台任务结束仍没下载完，取消
    
    /**
     * Handles cookies stored in NSHTTPCookieStore by setting
     * NSMutableURLRequest.HTTPShouldHandleCookies = YES;
     */
    SDWebImageHandleCookies = 1 << 5,           // 存储 handles cookies
    
    /**
     * Enable to allow untrusted SSL certificates.
     * Useful for testing purposes. Use with caution in production.
     */
    SDWebImageAllowInvalidSSLCertificates = 1 << 6, // 允许使用无效的 SSL 证书
    
    /**
     * By default, images are loaded in the order in which they were queued. This flag moves them to
     * the front of the queue.
     */
    SDWebImageHighPriority = 1 << 7,                // 移动到队列首位优先下载
    
    /**
     * By default, placeholder images are loaded while the image is loading. This flag will delay the loading
     * of the placeholder image until after the image has finished loading.
     */
    SDWebImageDelayPlaceholder = 1 << 8,            // 完成下载时展示 placeholder
    
    /**
     * We usually don't apply transform on animated images as most transformers could not manage animated images.
     * Use this flag to transform them anyway.
     */
    SDWebImageTransformAnimatedImage = 1 << 9,      // 转换动画图片
     ...
     */
    SDWebImageAvoidAutoSetImage = 1 << 10,          // 不自动设置图片
     ...
     */
    SDWebImageScaleDownLargeImages = 1 << 11,       // 按比例将减少下载图片的大小（bytes）
     ...
     */
    SDWebImageQueryMemoryData = 1 << 12,            // 强制查找磁盘数据
     ...
     */
    SDWebImageQueryMemoryDataSync = 1 << 13,        // 同步查找内存
     ...
     */
    SDWebImageFromLoaderOnly = 1 << 16,             // 直接加载，不查找缓存
     ...
     */
    SDWebImagePreloadAllFrames = 1 << 20,           // 预解压 animated image (Disk Cache & Download only)
     ...
     */
    SDWebImageMatchAnimatedImageClass = 1 << 21,    // 返回 AnimatedImage 的类型为提供的指定类型
     ...
     */
    SDWebImageWaitStoreCache = 1 << 22,             // 图片缓存到磁盘完毕再回调
     ...
     * Use this flag to transform them anyway.
     */
    SDWebImageTransformVectorImage = 1 << 23        // 转换成向量图
};
